...
public class CarNameValidator {
    private static final String START_OR_END_WITH_DELIM_MESSAGE = "[ERROR] 구분자가 맨 앞이나 맨 뒤에 있으면 안된다.\n";
    private static final String SERIAL_DELIM_MESSAGE = "[ERROR] 구분자가 연속으로 입력되면 안된다.\n";
    private static final String DUPLICATION_MESSAGE = "[ERROR] 동일한 자동차 이름이 있으면 안된다.\n";
    private static final String OVER_LENGTH_MESSAGE = "[ERROR] 자동차 이름은 5자 이하여야 한다.\n";
    private static final String WRONG_FORMAT_MESSAGE = "[ERROR] 자동차 이름은 한글, 알파벳, 숫자 조합으로만 이루어져야 한다.\n";
    ...
    private static void checkStartOrEndWithDelim(String name) {
        if (isStartOrEndWithDelim(name)) {
            throw new IllegalCarNameException(START_OR_END_WITH_DELIM_MESSAGE);
        }
    }
    ...
    private static void checkDelimIsSerial(String name) {
        if (isDelimSerial(name)) {
            throw new IllegalCarNameException(SERIAL_DELIM_MESSAGE);
        }
    }
    ...
    private static void checkDuplication(String eachName) {
        if (isSameName(eachName)) {
            throw new IllegalCarNameException(DUPLICATION_MESSAGE);
        }
    }
    ...
    private static void checkNotOverLengthOf(String eachName) {
        if (isLongerThanMax(eachName)) {
            throw new IllegalCarNameException(OVER_LENGTH_MESSAGE);
        }
    }
    ...
    private static void checkRightFormatOf(String eachName) {
        if (!isMatchRightFormat(eachName)) {
            throw new IllegalCarNameException(WRONG_FORMAT_MESSAGE);
        }
    }
