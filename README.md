...
## 🚀 기능 정리
- 경주할 자동차 이름을 입력받는 매소드
  - `경주할 자동차 이름을 입력하세요.(이름은 쉼표(,) 기준으로 구분)`가 먼저 화면에 출력되도록 한다.
  - 이름은 콤마(,)로 구분한다.
  - 입력은 `camp.nextstep.edu.missionutils`에서 제공하는 `Console` API를 활용한다.
  - 자동차의 이름(String)으로 이뤄진 배열 arrayCar를 return 합니다.
- 자동차 이름별로 Car 객체를 만드는 매소드
  - arrayCar 배열을 매개변수로 사용합니다.
  - for문으로 arrayCar의 요소를 하나씩 꺼냅니다.
  - 꺼낸 요소로 car 객체를 선언합니다.
- 시도할 횟수를 입력하는 매소드
  - `시도할 회수는 몇회인가요?` 가 먼저 화면에 출력되도록 한다.
  - 입력은 `camp.nextstep.edu.missionutils`에서 제공하는 `Console` API를 활용한다.
- 자동차 이름을 잘못 입력했을 때 나오는 매소드
  - `[ERROR]`를 나오게 하고, 다시 경주할 자동차 이름을 입력받는 매소드를 불러온다
- 횟수를 잘못 입력했을 때 나오는 매소드
