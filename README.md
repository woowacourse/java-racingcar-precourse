...
1. 인덴트는 2까지
2. 3항 연산자 사용 불가
3. 메소드의 길이가 15가 넘어가면 안됨
4. 함수는 한 가지의 일만
5. else를 사용하지 않는다.
6. Car는 주어진 객체를 사용. - 함수 추가 가능
7. Randoms, Console은 주어진 라이브러리 사용
8. 테스트는 성공해야함.
9. 함수와 클래스 분리가 충분한지
10. 이름을 통해 의도가 잘 들어났는지
11. 메서드 이름이 한 두 단어인지, 문맥의 중복이 있는지.
12. 문맥 분리로 공백을 사용하는지
13. 의미있는 주석을 달았는지
14. 기능목록 업데이트가 됐는지
15. 기능목록에 예외경우도 있는지
16. 리드미에 설명, 프로젝트 구조, 기능목록, 실행방법, 기능설명이 적혀있는지
17. 코드 정렬을 했는지 (Ctrl+Alt+L)
18. 매직넘버를 사용하지 않는지. define class
19. 구현 순서가 지켜졌는지. 상수, 클래스 변수 - 인스턴스 변수 - 생성자 - 메서드
