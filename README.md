...
- 출력
  - 매 진행마다 진행 상황을 출력한다.
    - 이 때 진행 상황은 '-'로 출력한다.
    - 진행 상황과 함께 경주할 자동차의 이름을 함께 출력한다.
  - 게임이 다 진행된 이후(시도 횟수 만큼) 우승자를 출력한다
    - 여러 명이면 ','로 분리한다
  - 사용자 입력이 잘못된 경우
    - `IllegalArgumentException` 발생 이후 입력을 다시 받는다.

- 게임 진행
  - 매 게임 진행마다 각 자동차 별로 0에서 9의 무작위 값을 구한다.
    - 4 이상일 경우에만 자동차는 한 칸 전진한다.
    - 이외에는 멈춘다.

- 예외 처리
  - "[ERROR]"로 시작하는 적절한 예외 메시지 출력
