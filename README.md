...
- 자동차들의 이름을 입력 받는다.
  - [ ] 자동차들의 이름 입력은 , 으로 각 자동차를 구분한다.
  - [ ] 자동차의 이름 길이는 1 ~ 5 사이의 길이로 구성되어야 한다.
  - [ ] (예외) 자동차의 입력은 공백을 허용하지 않는다.
  - [ ] (예외) 자동차의 입력은 빈 입력을 허용하지 않는다.
  - [ ] (예외) 자동차들의 입력 중 중복된 이름의 입력은 허용하지 않는다.



- 자동차 경주 게임의 시도할 횟수(라운드)를 입력 받는다.
  - [ ] (예외) 시도할 횟수는 숫자 형태로만 입력을 허용한다.
  - [ ] (예외) 숫자는 1 이상의 숫자를 허용한다.


- 각 자동차의 전진을 결정한다.
  - [ ] 자동차 전진을 위해서 값을 통해 전진을 수행한다.
    - [ ] 값은 랜덤한 값으로 생성하며 0 ~ 9 사이의 값으로 생성 할 수 있다.
  - [ ] 값이 4이상인 경우만 자동차가 전진을 할 수 있다.
  - [ ] 각 자동차는 모두 다른 랜덤 값을 가진다.
  

- 각 라운드가 끝나면 현재 결과를 출력해야 한다.
  - [ ] '이름 : 전진 횟수(-)' 순으로 출력해야 한다.


- 입력한 시도 횟수(라운드)가 모두 끝이나면 우승자를 판별한다.
  - [ ] 가장 전진을 많이 한 자동차를 우승자로 판별한다.


- 판별된 우승자를 출력한다.
  - [ ] 우승자는 최소 1 이상이다.
  - [ ] 우승자가 여려명일 경우 ' ,' 형태로 우승자를 구분한다.
