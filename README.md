...
- 올바르지 않은 이름을 입력받을 경우 재시작
- 사용자로부터 이동 횟수를 입력
- 무작위 수를 생성하고 전진 혹은 정지
- 차수별 실행 결과를 출력
- 우승자를 확인하고 출력
---
## 리팩토링할 코드 리스트
- 변수명에 자료형이 들어가지 않도록 수정
- 상수들을 Constant 클래스가 아닌 각기 필요한 클래스에서 관리
- 재귀 로직을 반복문으로 수정
- 우승자 출력 방법 변경
- 테스트 코드에서 Reflection 사용하지 않도록 로직 수정
---
## 구현할 클래스 리스트
- Game
  - 게임 진행을 담당하는 클래스
  - 이름과 게임 실행 횟수를 입력받고 유효성 검사
  - 게임 실행 횟수만큼 게임 진행
  - 매 회차 게임 진행 시 무작위 수를 생성
- Car
  - 자동차 클래스
  - 이름, 위치를 필드로 가짐
  - 무작위 수를 받아서 조건에 맞으면 전진
  - 각 회차마다 본인의 이름과 위치를 출력
- Cars
  - Car들을 모아놓은 컬렉션
  - 자동차를 생성하고 우승자를 구하는 등 자동차들에 관련된 로직 담당
- InputHandler
  - 입력을 담당할 클래스
- Validator
  - 입력의 유효성을 체크하는 클래스
