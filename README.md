...
### 게임 세팅
- [x] 경주 할 자동차를 생성한다.
- [x] 각 자동차의 전진을 위한 0~9 범위의 랜덤 숫자를 생성한다.
  - 랜덤 값이 4 이상일 경우에만 전진한다.
  - 랜덤 값이 4 미만인 경우에는 멈춘다.
- [x] 시도할 횟수만큼 반복한 후 게임을 종료한다.

### 입력
- [x] 경주 할 자동차 이름을 쉼표 기준으로 구분하여 입력받는다.
- [x] 시도할 횟수를 1 이상의 숫자로 입력받는다.
...
- [x] 각 차수별 실행 결과를 출력한다.
  ...
  - 위치 수 만큼 '-' 문자로 표시한다.
- [x] 최종 우승자를 출력한다.
  - 공동 우승자가 있는 경우 각 자동차의 이름을 ', ' 문자열로 구분한다.

### 예외 상황

#### 경주 할 자동차 이름에 대한 사용자의 입력
- 각 자동차의 이름이 1자 이상, 5자 이하가 아닌 경우
- 자동차의 이름이 공백 문자이거나 포함된 경우
- 문자열 마지막에 쉼표가 있는 경우
- 자동차 이름 문자열 중 중복된 문자열이 있는 경우
- 하나의 자동차만 입력한 경우 (경주 할 대상이 없음)

#### 시도할 횟수에 대한 사용자의 입력
- 숫자 문자열이 아닌 경우
- 0 이하의 숫자인 경우 (0 혹은 음수)
