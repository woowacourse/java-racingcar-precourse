...
#### 😀RaceGame class 
- 전반적으로 게임관련 기능들에 대한 책임을 가진다.
    - 사용자의 입력을 받는 기능과 예외발생시 예외를 처리할방법에 대한 권한또한 가진다. 그러나 예외 발생에 대한 권한은 InputChecker에게 위임한다.
    - 출전자동차들을 게임에 생성하고 관리할 책임
      - 이 책임은 추후에 우승자 선발에 대한 책임과도 연관이 생긴다.
    - 경기진행에 대한 책임  
        - 자동차들에게 움직여라! 라고 명령할 권한만 있을뿐 움직임이란 행위는 자동차에게 위임한다.
        - 자동차들의 상태를 출력할 시기에 대한 책임또한 진다. 코드는 움직여라 이후에 바로 상태를 출력하라 라고 메세지를 자동차들에게 보낸다.
    - 우승자 선발대한 책임
        - 가장 멀리간 차를 구하는 과정을 결정할 권한이 있다.
        - 시도횟수에 따른 우승자를 선출한다. 시도횟수가 0인경우도 예외 없이 처리한다.
