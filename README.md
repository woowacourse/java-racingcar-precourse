...
### 1. 입력을 통해 차의 이름과 시도 횟수 읽기

  - 쉼표를 split 하여 이름을 얻고 예외처리 이후 자동차 객체로 생성

  - 시도횟수를 입력 받고 예외처리 이후 저장

### 2. 시도 횟수마다 random을 통해 전진/후진 결정

###3. 시도 횟수 종류 후 가장 많이 이동한 차를 찾아 출력

- Car 객체에서 getPosition을 통해 이동 횟수를 받기
- 임의의 max값을 받는 변수를 0으로 초기화 하여 각 Car객체를 돌아보며 최대 이동횟수를 구함
- Car 객체의 getPosition과 max변수를 비교하여 같으면 우승자에 포함
