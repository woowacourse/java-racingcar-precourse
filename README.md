...
- Car 객체에 이름, 전진, 랜덤값이 담긴 클래스
  - random 값이 4이상인 경우 position 상승
  - position 상승에 의한 자동차 이름과 이동거리 출력("-")
- Cars Car 객체들을 리스트로 담긴 클래스
  - 사용자 input count횟수에 대하여 자동차들 레이싱 
- InputView 사용자의 입력 클래스
  - input 자동차 이름을 배열화
  - input 레이싱을 할 횟수
- Game 레이싱 진행과 우승자를 출력하는 클래스
- Message 게임 진행 시 출력 되는 문자열을 담은 클래스
  - Game 가이드 및 에러 메시지
- ValidateUtils 사용자의 Input값에 대한 예외처리
  - 자동차 이름을 5글자 이상 입력 했을 시 예외처리
  - 자동차 이름을 생략 했을 시 예외 처리
  - 자동차  이름이 중복 됐을 시 예외처리
  - 레이싱 횟수가 0을 입력했을 시 예외처리
## 🚀 기능 요구사항
