...
- 입력 및 예외처리
  - 경주할 자동차들의 이름들을 입력
    - 입력한 이름 중 5자를 초과하는 경우 에러
    - 숫자, 영어 알파벳, 한글(단일 자음, 모음 허용 x)외의 문자를 입력하는 경우 에러
    - 입력한 이름 중 이름의 길이가 0자인 경우 에러 ex)"pobi,,crong"
    - 입력된 이름이 2개 미만일 경우 에러
    - 중복된 이름을 입력할 경우 에러
    - 입력에서 예외가 발생한 경우 에러 메시지를 출력하고 재입력
  - 시도할 회수 입력
    - 숫자 외의 문자를 입력하는 경우 에러
    - 숫자가 양의 정수가 아닌 경우 에러
    - 입력에서 예외가 발생한 경우 에러 메시지를 출력하고 재입력

- 자동차 전진 기능
  - 0 ~ 9사이의 값 중 랜덤으로 하나 뽑아 4 이상일 경우 전진

- 출력
  - 각 차수별 자동차 위치 출력
  - 경주가 끝난 후, 우승자를 출력
    - 공동 우승자가 존재할 경우 ','로 구분해 함께 출력
    
- 전체 게임 컨트롤러
  - 자동차 이름들을 입력받아 자동차 객체를 생성
  - 입력 회수를 입력받아 회수만큼 자동차를 전진, 결과 출력
  - 경주가 끝난 후, 우승자를 출력
