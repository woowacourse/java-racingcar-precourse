...
3. 전진하는 조건에 따라서 자동차의 위치를 출력하는 기능
    - 0부터 9까지 랜덤값을 생성한다.
    - 랜덤값이 4이상일 경우 전진한다.
    - 랜덤값이 3이하일 경우 정지한다.
4. 최종 우승자를 출력하는 기능
    - 최대 위치를 구한다.
    - 최대 위치와 위치가 같은 자동차를 우승자로 정한다.
5. 예외처리
    - 자동차이름 앞뒤로 공백이 있을 경우 제거한다.
    - 자동차이름이 5자를 넘을 경우 5자까지만 출력한다.
    - 자동차이름이 빈칸일 경우 미입력으로 간주한다. 
    (ex : pobi, ,woni -> pobi,woni)
    - 이동횟수가 숫자형식이 아닐때 다시 입력한다.
    - 이동횟수가 음수일 때 절대값이 입력된다.
    - 경주할 자동차가 없을 경우 다시 입력한다.
