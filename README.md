...
1.  자둥차 이름 확인
    - 사용자로 부터 자동차 이름을 입력 받는다.
    - [Exception] 자동차 이름 길이 확인 : 자동차 이름은 쉼표(,)를 기준으로 구분하며 이름은 5자 이하만 가능하다.
  
2. 이동 횟수 확인
    - 사용자로 부터 이동할 횟수를 입력 받는다.
    - [Exception] 이동횟수 범위 확인 : 이동 횟수는 양의 정수로 이루어 져야 한다.
...
4. 자동차의 현재 위치 출력
    - 한 턴이 지날 때 마다 자동차의 현재 위치를 표시해준다. (print)
