...
2020-12-05 오전 2시 23분 시작

## 기능 목록

- ####  입력
    - 사용자로부터 자동차 이름을 입력받는다. (쉼표로 구분한다.)'
    - 이동을 시도할 횟수를 입력 받는다. (음수가 아닌 정수)

- #### 출력
    - 매 라운드 별 자동차의 위치를 출력한다.
    - 최종 우승자를 출력한다.
    - 예외 발생의 경우 에러 문자를 출력한다. ( ERROR로 시작한다.)

- #### 예외 처리
    - 자동차 이름이 없거나, 길이가 0인 경우
    - 자동차 이름이 6글자 이상인 경우
    - 자동차 이름에 중복이 있는 경우
    - 사용자 시도 횟수가 음수 또는 정수가 아닌 경우
    
- #### 레이싱 게임 룰 구현
    - 자동차는 매 라운드 별로 전진을 시도한다.
    - 자동차는 random한 0~9 사이의 값을 얻고, 그 값이 4이상인 경우 전진한다.
    - 라운드 종료 시 가장 많이 전진한 자동차를 우승 자동차로 한다. (다수 가능)
...
- #### racingcar
    - Application : 자동차 이름과 경주 라운드 수를 결정하고, RacingGame 실행을 결정한다. 
