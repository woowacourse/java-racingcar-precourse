...
- 게임 시작 기능
  -  게임 시작
- 사용자 입력 기능
  - 경주 할 자동차 이름 입력(쉼표(,) 기준으로 구분)
  - 시도할 횟수 입력
  - 밑의 `예외 처리 기능`을 고려하여 입력 받도록 함
  - 사용자가 잘못된 값을 입력할 경우 IllegalArgumentException를 발생시키고, "[ERROR]"로 시작하는 에러 메시지를 출력 후 그 부분부터 입력을 다시 받는다.
- 경주 실행 기능
  - 사용자가 입력한 횟수 만큼 반복 실행
    - 0부터 9까지의 랜덤 숫자를 뽑아 4 이상인 경우 1보 전진
    - 각 차수별 자동차 위치 출력
- 최종 우승자 츨력 기능
  - 각 자동차의 위치를 확인 후 가장 멀리 이동한(Position 값이 가장 큰) 자동차를 출력
  - 우승자가 여러 명일 경우, 쉼표(,)를 이용하여 구분
- 예외 처리 기능
  - 자동차 이름은 5글자 이하로만 구성
  - 중복되지 않는 서로 다른 자동차 이름으로 구성
  - 공백이 아닌 자동차 이름으로 구성
  - 자동차가 최소 2대 이상이어야 함
  - 시도 횟수는 숫자이어야 함
