...
2. 1의 입력에 대한 유효성 검사 및 예외처리를 수행한다.

   - [예외처리]  입력 값이 비어있을 때
   - [예외처리]  입력 값이 쉼표(,)만 있을 때
   - [예외처리]  입력 값에 쉼표가 없고 이름이 오로지 1개 뿐일 때
   - [예외처리]  구분된 자동차의 이름이 비어있을 때
   - [예외처리]  구분 된 자동차의 이름의 길이가 6 이상일 때
   - [예외처리]  구분 된 자동차의 이름 중에서 중복된 자동차 이름이 있을 때


3. 이름이 부여된 자동차들을 생성시킨다.


4. 경주할 자동차들이 이동을 시도할 횟수를 입력받는다.


5. 4의 입력에 대한 유효성을 검사 및 예외처리를 수행한다.

   - [예외처리] 입력 값이 정수가 아닐 때
   - [예외처리] 입력 값이 정수이지만 0이거나 음수일 때
   - [예외처리] 입력 값이 정수이지만 허용된 범위를 벗어났을 때
   - [예외처리] 입력 값이 비어있을 때


6. 자동차가 조건(0에서 9사이의 무작위 값이 4 이하인 경우)을 만족하면 전진하게 한다.


7. 이동을 시도할 횟수 만큼 경주 게임을 진행하고 각 자동차의 전진여부를 결정한다.


8. 차수별 결과를 출력한다.(자동차 별로 "이름 : ---.."의 형태로 출력한다. "-" 는 위치 1에 해당하는 거리다.)


9. 최종 우승자를 가릴 수 있도록 가장 멀리 이동한 자동차의 위치를 구해 온다.


10. 9의 결과를 기준으로 같은 거리만큼 이동한 자동차가 우승자가 되게 한다.
